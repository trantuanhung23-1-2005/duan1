...
class UserAdminController {
    ...
    public function addUser() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $success = $userModel->addUserToDB($name, $email, $password, $role, $gender);
    ...
    public function addPostUser() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($_POST['name']) || empty($_POST['email']) || empty($_POST['password']) || empty($_POST['role']) || empty($_POST['gender'])) {
                $_SESSION['message'] = "Vui lòng điền đầy đủ thông tin!";
                header("Location: " . BASE_URL . "?role=admin&act=list-add");
                exit;
            }

            $success = $userModel->addUserToDB($_POST['name'], $_POST['email'], $_POST['password'], $_POST['role'], $_POST['gender']);
  ...
  public function editPostUser() {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (empty($_POST['id']) || empty($_POST['name']) || empty($_POST['email']) || empty($_POST['role']) || empty($_POST['gender'])) {
            $_SESSION['message'] = "Vui lòng nhập đầy đủ thông tin!";
        ...
        // Gọi update
        $success = $userModel->updateUserToDB($_POST['id'], $_POST['name'], $_POST['email'], $_POST['role'], $_POST['gender'], $password);
